...
StringArray = [['filename', 'line', 'index/pos', 'type', 'string']] # format: filename, line, quotIndex, quotType, string
    ...
    for line in lines:
        ...
        for mo in inString.finditer(line):
            if len(mo.group("words")) > 0:
                StringArray += [[filename, lineCount, quotIndex, quotType(mo.group("quote")), mo.group("words")]]
            quotIndex += 1
    alllines = '\n'.join(lines)
...
def saveTheWorld():
    ...
    for line in StringArray:
        SaveString += '"' + line[0] + '";' + repr(line[1]) + ';' + repr(line[2]) + ';' + line[3] + ';"' + line[4].replace('"', '""') + '"\n'
    f = open(outputFile, 'w')
    f.write(SaveString)
    f.close()
